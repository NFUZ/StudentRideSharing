...
app.post("/api/signup", (req, res) => {
  ...
  if (!email.endsWith("@sm.imamu.edu.sa")) {
    return res.status(400).json({
      error:
        "Please use your university email (@sm.imamu.edu.sa).",
    });
  }
...
app.post("/api/rides", (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      error:
        "driverId, from, to, time, seats, and basePrice are required.",
    });
  }

  const perRider =
    Number(basePrice) / Number(seats);
...
app.post("/api/requests", (req, res) => {
  ...
  if (!rideId || !studentId) {
    return res.status(400).json({
      error:
        "rideId and studentId are required.",
    });
  }

  const ride = rides.find(
    (r) => r.id === Number(rideId)
  );
...
app.post("/api/payment", (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      error:
        "rideId, studentId, amount, and method are required.",
    });
  }
...
app.post("/api/sos", (req, res) => {
  ...
  res.json({
    message: "SOS report stored (mock)",
    report,
  });
});
...
app.listen(PORT, () => {
  console.log(
    `✅ Student Ride-Sharing API running on http://localhost:${PORT}`
  );
});